...
class WSLP( SerialDevice ):
    ...
    def generateSweepProfile(self, numSweeps, numSteps, numDwells, numGroups ): # added numGroups to list
        print("generateProfile() method called from WSLP")
        self.generateSweepLeg(numSteps)
        
        
        dwell_list = np.fromiter( (2 for a in range(numDwells)), int )
        group_list = np.tile(np.array([1,0]), numSweeps)
        if self.dwellSide == 'electron':
            group_list = group_list ^ 1
        if self.dwellSide == 'alt':
            group_list = np.append( group_list, np.asarray(group_list[-1]^1) )
        
        # self.sweepProfile = np.tile( np.append( (group_list^1 if self.dwellSide == 'alt' else group_list), dwell_list), numGroups)
        
        self.sweepProfile = np.array([])
        for i in range(numGroups):
            self.sweepProfile = np.append( self.sweepProfile, np.append(group_list, dwell_list) )
            if self.dwellSide == 'alt':
                group_list =  group_list^1
                
        
            
    def identifySweepPoints(self):
        # create list from lots of iteration
        # return as np.array ?
        pass
        
        
        
        # print(self.sweepProfile[i])
    ...
    def doProgramSweep(self, numSweeps, numSteps, numDwellsOrGroups, numSamples ):
        ...
        if self.cmdByte( SOH ):
            ...
            if self.cmdByte( STX ):
                ...
                if self.cmdByte( ETX ):
                    for sweepProfile in self.sweepProfileList:
                        self.writeByte( sweepProfile )
    ...
    def parseHeader(self, data):
        ...
        if boardID == 1:
            ...
            headerData = {
                "boardID" : int(data[5]),
                "sweepDirection" : sweepDir,
                "sweepIndex" : int( np.uint16( data[7])<< 8 | np.uint16(data[8]) ),
                "numSweeps" : int(  np.uint16( data[9]) <<8 | np.uint16(data[10]) ),
                "numSteps" : int(   np.uint16( data[11])<<8 | np.uint16(data[12]) ),
                "numSamples" : int(data[13]),
                "DACstart" : np.uint16(data[14])<<8 | np.uint16(data[15]),
    ...
    def doTakeSweep(self, saveFlag, systemData):
        ...
        if not self.validateConnection():
            ...
            try:
                timePerStep = 50e-6 # 33.3333 µs for Fs = 30kHz; 50µs for 20kHz
